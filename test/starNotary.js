...
it("token name and symbol", async () => {
  let tokenName = "Star Token Registry";
  let tokenSymbol = "STR";
...
it("exchange stars between two users", async () => {
  let user1 = accounts[1];
  let user2 = accounts[2];
  let starId1 = 7;
  let starId2 = 8;
  await instance.createStar("my awesome star", starId1, { from: user1 });
  await instance.createStar("my awesome star - with glitter", starId2, {
    from: user2
  });
...
it("Star tokens can be transferred from one address to another", async () => {
  let user1 = accounts[0];
  let user2 = accounts[2];
  let starId = 6;
  await instance.createStar("my awesome star", starId, { from: user1 });
  await instance.transferAStar(user2, starId);
